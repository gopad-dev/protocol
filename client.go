...
func clientDispatch(ctx context.Context, client Client, reply jsonrpc2.Replier, req jsonrpc2.Request) (handled bool, err error) {
	...
	case MethodInlayHintRefresh:
		defer logger.Debug(MethodInlayHintRefresh, Error(err))
...
type Client interface {
	...
	InlayHintRefresh(ctx context.Context) (err error)
}
...
// InlayHintRefresh sends the notification from the server to the client to ask the client to refresh inlay hints.
func (c *client) InlayHintRefresh(ctx context.Context) (err error) {
	c.logger.Debug("call " + MethodInlayHintRefresh)
	defer c.logger.Debug("end "+MethodInlayHintRefresh, Error(err))

	return c.Conn.Notify(ctx, MethodInlayHintRefresh, nil)
}
